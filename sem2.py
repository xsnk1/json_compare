...
def difference(dict_1,dict_2,path): #функция нахождения различий
    result = '' 
    for key in dict_1:  #с помощью рекурсии мы проверяем изменения вложенных ключей 
        if key in dict_2:
            if type(dict_1[key]) is dict:
                path += str(key) + '.'
                result += difference(dict_1[key],dict_2[key],path)
                path = path.replace(f'{str(key)}.','')

                
    for key in list(dict_1): #проверяем какие ключи были удалены из первого  файла
        if key not in dict_2:
            result += f'Ключ "{path + str(key)}" удален\n'
            dict_1.pop(key)


    for key in dict_2:  # проверяем какие ключи были добавлены во второй  файл
        if key not in dict_1 :
            if isinstance(dict_2.get(key), (dict,list)):

                result += f'Ключ "{path+str(key)}" добавлен: "[complex value]"\n'

            elif type(dict_2[key]) != dict:

                result += f'Ключ "{path+str(key)}" добавлен: "{dict_2.get(key)}"\n'

    for key in dict_1:    # проверяем какие ключи были изменены
        if dict_1.get(key)!=dict_2.get(key) and type(dict_1[key]) != dict:

            result += f'Ключ "{path+str(key)}" изменён. Было "{dict_1.get(key)}". Стало "{dict_2.get(key)}"\n' # после выполнения данных циклов мы получаем строку в которой написаны все изменения наших файлов
...
print(create_file(difference(dict_1,dict_2,path)))
